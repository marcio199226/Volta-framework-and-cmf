...
class Vf_register_Component extends Vf_Controller
{
			...
			if($csrf -> csrf_check_token($request -> post('csrf_token')))
			{
				...
				if(sizeof($validation -> get_errors()) == 0)
				{
					...
					if($registerModel -> isSaved())
					{
						...
						$apiKey = Vf_Orm::factory('RestfulApiKey');
						...
						if($registerCfg -> account_activate == 0)
						{
							...
							$view -> success_add_user = 'Zostales zarejestrowany, mozesz sie zalogowac';
						}
						else
						{
							...
							$notify -> setSubject('Link Aktywacyjny');
							$notify -> setUrl('<a target="_blank" href="http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].',activeAccount,'.$code.'">Aktywuj konto</a>'); 
							$notify -> setMessage('By aktywowac konto kliknij na odnosnik: '.$notify -> getUrl());
							$notify -> notify();
							$registerModel -> addAccountData($account);
							$view -> success_add_user = 'Zostales zarejestrowany, na twoj adres email zostal wyslany link aktywujacy konto';
						}
					}
					else
					{
						$view -> error_add_user = 'Wystapil blad podczas rejestracji';
					}
				}
				else
				{
	...
	public function activeAccount()
	{
		...
		{
			throw new Vf_Component_Exception('Podany kod aktywacyjny jest nieprawidlowy');
		}
		...
		if(!$isActive)
		{
			if($accountModel -> activeAccount($this -> uri -> getSegment(3)))
			{
				...
				$view -> loadHelper('Box');
				$view -> isDisactive = 'Konto zostalo aktywowane teraz mozesz sie zalogowac';
				return $view -> render();
			}
		}
		else
		{
			$view = new Vf_View('accountActivateConfirm', 'component', 'register');
			$view -> loadHelper('Box');
			$view -> isActive = 'To konto zostalo juz wczesniej aktywowane';
			return $view -> render();
		}
	}
}
