...
class adminPages_Commons
{
	...
	public function checkCompressionLib()
	{
		...
		else if(extension_loaded('Zip') && extension_loaded('Rar'))
		{
			return 'both';
		}
	...
	public function extractTranslations($package)
	{
		...
		{
			$type = $this -> _checkCompressionLib();
	...
	public function extractWidgetBundle($bundle)
	{
		...
		if($extracted)
		{
			if(copy(DIR_WIDGETS.$bundleName[0].'/'.DIR_CTRL.$bundleName[0].'.php', DIR_WIDGETS_CTRL.$bundleName[0].'.php') && copy(DIR_WIDGETS.$bundleName[0].'/'.DIR_VIEWS.$bundleName[0].'.php', DIR_WIDGETS_VIEWS.$bundleName[0].'.php'))
			{
