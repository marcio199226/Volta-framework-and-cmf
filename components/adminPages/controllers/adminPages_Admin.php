...
class Vf_adminPages_Admin_Component extends Vf_Controller
{
	...
	public function usersManager()
	{
		...
		if ($this->request->post('submit_search_user')) {
			...
			} elseif ($this->request->post('action_type') == 'unban_user') {	
			...
			} elseif ($this->request->post('action_type') == 'active_account') {
			...
			} elseif ($this->request->post('action_type') == 'disable_account') {
				$user_id = $this->request->post('user_id');
				$data = array(
					'login' => $user,
					'code' => substr(sha1(time() . uniqid()), 0, 20),
